...
class CombatUI {
      ...
      entityEl.innerHTML = `
        ...
        <div class="entity-health">
          ...
          <div class="health-bar-container">
            <div class="health-bar" style="width: ${entity.getHealthPercentage()}%"></div>
          </div>
        </div>
        <div class="entity-energy">
          <div>Energy: ${entity.energy}/${entity.maxEnergy}</div>
          <div class="energy-bar-container">
            <div class="energy-bar" style="width: ${entity.getEnergyPercentage()}%"></div>
          </div>
        </div>
        <div class="entity-action-points">
          AP: ${Math.floor(entity.actionPoints)}/${entity.maxActionPoints}
        </div>
      `;
    ...
    updateEntityElement(entity) {
      ...
      healthBarEl.style.width = `${entity.getHealthPercentage()}%`;
      ...
      energyBarEl.style.width = `${entity.getEnergyPercentage()}%`;
      entityEl.querySelector('.entity-energy > div').textContent = `Energy: ${entity.energy}/${entity.maxEnergy}`;
      
      // Update action points
      entityEl.querySelector('.entity-action-points').textContent = `AP: ${Math.floor(entity.actionPoints)}/${entity.maxActionPoints}`;
    ...
    renderCombatLog(logEntries) {
      // Clear existing log if full refresh
      if (logEntries.length > 0 && logEntries[0].time === logEntries[0].time) {
        this.combatLogContainer.innerHTML = '';
      }
    ...
    updateActionPoints() {
      ...
      // Update text
      this.actionPointsCount.textContent = Math.floor(localPlayer.actionPoints);
      this.maxActionPoints.textContent = localPlayer.maxActionPoints;
      
      // Update progress bar
      const percentage = localPlayer.getActionPointsPercentage();
      this.actionPointsBar.style.width = `${percentage}%`;
    ...
    updateActionButtons() {
      ...
      const canAct = localPlayer.canAct();
